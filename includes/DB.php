...
class DB extends Conexion{
   ...
   protected static function ejecutaConsulta($sql,$parametros) {
     ...
     * @return mixed
     */
    public function getUsuario($email,$tipo,$contrasena=null){
        try{
    ...
    public function crearNuevoUsuario($email,$contrasena,$tipo){
        ...
        try{
            ...
            $conexion->commit();
            $usuario=$this->getUsuario($email,$tipo,$contrasena);
            return $usuario;
        }catch(PDOException $e){
            $conexion->rollBack();
            return false;
    ...
    public function getAdministrador($idusuario){
        try{
            $sql = "SELECT * FROM administradores as t1 INNER JOIN usuarios as t2 
